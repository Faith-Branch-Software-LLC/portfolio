  ...
  return (
    <>
      {sections.map((sectionContent, index) => {
        ...
        const textColor = isRedSection ? 'white' : 'black';

        return (
          <BlogSection key={index} sectionIndex={index}>
            <div className="container mx-auto px-4 py-8">
              <div
                className={`prose prose-lg max-w-none prose-headings:font-black prose-a:bg-darkPurple prose-a:text-white prose-a:px-2 prose-a:py-1 prose-a:rounded-md prose-a:shadow-button prose-img:rounded-lg prose-img:max-h-[350px] prose-img:w-auto prose-pre:bg-[#002B36] prose-pre:text-[#eee8d5] prose-pre:rounded-lg prose-pre:shadow-card prose-pre:p-4 prose-li:marker:text-${textColor} prose-ol:list-decimal prose-ul:list-disc font-gelasio ${
                  isRedSection
                    ? 'prose-headings:text-white prose-p:text-white prose-li:text-white prose-strong:text-white prose-em:text-white'
                    : 'prose-headings:text-black prose-p:text-black prose-li:text-black prose-strong:text-black prose-em:text-black'
                }`}
      ...
      <Footer layer={sections.length + 1} />
    </>
  );
}
